...
from skillnecting import app, db, bcrypt
from skillnecting.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm
...
@app.route("/debugme", methods=['GET', 'POST'])
def debugme():
...
@login_required
def new_post():
    """Function for posting adding new post to blog"""
    form = PostForm()
    if form.validate_on_submit():
